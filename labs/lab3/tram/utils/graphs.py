...
class Graph:
    ...
    def add_edge(self, a, b):
        # this if statement is equivalent with the one below
        if a!=b and (b not in self._adjacencylist or (b in self._adjacencylist and a not in self._adjacencylist[b])):
        #if a != b and (a, b) not in edges and (b, a) not in edges: #leads to a bad time complexity when the graph is quite big
            self.add_vertex(a)
            self.add_vertex(b)
...
def dijkstra(graph, source, cost=lambda u,v: 1):
    ...
    # every node and its previous node to the shortest path
    ...
    dist[source] = 0
    #print(dist[source])
    #print(source in unvisited_nodes)
    ...
    while unvisited_nodes:
        ...
        for neighbor in graph.neighbors(current_node):
            ...
            #print(str(current_node) + '  '+str(dist[current_node]))
